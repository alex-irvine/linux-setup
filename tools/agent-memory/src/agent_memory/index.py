...
class MemoryIndex:
        ...
        with self._connect() as connection:
            connection.execute("""INSERT OR REPLACE INTO memories VALUES
                (:id, :type, :scope, :project, :content, :importance, :confidence, :pinned,
                 :tags, :revision, :content_hash, :created_at, :updated_at)""", values)
    ...
    def idempotency(self, request_id: str) -> tuple[str, str] | None:
        with self._connect() as connection:
            return connection.execute("SELECT payload_hash, result_id FROM idempotency WHERE request_id = ?", (request_id,)).fetchone()

    def remember_idempotency(self, request_id: str, payload_hash: str, result_id: str) -> None:
        with self._connect() as connection:
            connection.execute("INSERT INTO idempotency VALUES (?, ?, ?)", (request_id, payload_hash, result_id))

    def list(self, query: ListQuery) -> list[MemoryRecord]:
        clauses, values = [], []
        for name in ("project", "scope", "type"):
    ...
    def _record(row: tuple) -> MemoryRecord:
        ...
        return MemoryRecord(UUID(row[0]), row[1], row[2], row[3], row[4], row[5], row[6], bool(row[7]), tuple(filter(None, row[8].split(","))), row[9], row[10], row[11], row[12])
